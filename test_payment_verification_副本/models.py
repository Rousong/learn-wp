...
class PaymentRecord(models.Model):
    ...
    class Meta:
        verbose_name = '测试支付记录'
        verbose_name_plural = '测试支付记录'
        ordering = ['-created_at']
        db_table = 'test_payment_verification_paymentrecord'
        indexes = [
    ...
    def __str__(self):
        return f"[TEST] {self.platform} - {self.product_type} - {self.transaction_id}"
...
class VerificationLog(models.Model):
    ...
    class Meta:
        verbose_name = '测试验证日志'
        verbose_name_plural = '测试验证日志'
        ordering = ['-created_at']
        db_table = 'test_payment_verification_verificationlog'
    
    def __str__(self):
        status = "成功" if self.is_successful else "失败"
        return f"[TEST] {self.payment_record.transaction_id} - {status} - {self.created_at}"
...
class UserSubscription(models.Model):
    ...
    class Meta:
        verbose_name = '测试用户订阅'
        verbose_name_plural = '测试用户订阅'
        db_table = 'test_payment_verification_usersubscription'
    
    def __str__(self):
        return f"[TEST] {self.user_id} - {'付费' if self.is_premium else '免费'}"
