...
class PaymentRecordAdmin(admin.ModelAdmin):
    ...
    def receipt_data_display(self, obj):
        ...
        if obj.receipt_data:
            ...
            return format_html('<pre style="max-width: 400px; overflow: auto;">{}</pre>', truncated)
        return '无'
    ...
    def verification_response_display(self, obj):
        ...
        if obj.verification_response:
            try:
                ...
                return obj.verification_response
        return '无'
...
class UserSubscriptionAdmin(admin.ModelAdmin):
    ...
    def current_subscription_link(self, obj):
        ...
        if obj.current_subscription:
            url = reverse('admin:payment_verification_paymentrecord_change', 
                         args=[obj.current_subscription.id])
            return format_html('<a href="{}">{}</a>', url, obj.current_subscription.transaction_id)
        return '无'
    current_subscription_link.short_description = '当前订阅'
    
    def subscription_count(self, obj):
        """显示订阅历史数量"""
        return obj.subscription_history.count()
    ...
    def update_subscription_status(self, request, queryset):
        ...
        self.message_user(request, f'成功更新 {updated_count} 个用户的订阅状态')
    update_subscription_status.short_description = '更新订阅状态'
