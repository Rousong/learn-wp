...
class AndroidPaymentService extends PaymentService {
  ...
  Future<bool> initialize() async {
    try {
      ...
      if (!available) {
        LogService.instance.e('Android 应用内购买不可用');
        return false;
      }
      ...
      LogService.instance.i('Android 支付服务初始化成功');
      return true;
    } catch (e) {
      LogService.instance.e('Android 支付服务初始化失败: $e');
      return false;
    }
  }
  ...
  Future<PaymentResult> purchaseProduct(String productId) async {
    try {
      ...
      if (success) {
        ...
        // 实际的购买结果会在 _onPurchaseUpdate 中处理
  ...
  Future<bool> verifyReceipt(String receiptData) async {
    try {
      ...
      LogService.instance.i('Android收据验证结果: ${result.isValid}');
      
      if (result.isValid && result.isActive) {
        // 验证成功且订阅有效，可以在这里更新本地用户状态
        await _updateLocalUserStatus(result);
        return true;
      } else {
        LogService.instance.w('Android收据验证失败: ${result.errorMessage}');
        return false;
      }
    } catch (e) {
      LogService.instance.e('Android收据验证异常: $e');
      return false;
    }
  }
